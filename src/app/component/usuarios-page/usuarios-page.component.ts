...
export class UsuariosPageComponent implements  OnInit, AfterViewInit {
  ...
  ngOnInit() {
    this.dataSource = new UsuariosDataSource(this.apiService, this.spinner);
  ...
  initForms() {
    ...
    const controlsConfig = {
      ...
      usuarioNmbre: [null, Validators.compose([Validators.required])],
      usuarioContrasena: [null, Validators.compose([Validators.required])],
      usuarioEstado: [null, Validators.compose([Validators.required])],
      codigoRol: [null, Validators.compose([Validators.required])],
  ...
  create() {
    ...
    this.spinner.show();
    this.apiService.usuarios.create(params).subscribe(() => {
        this.spinner.hide();
  ...
  startEditing(row) {
    ...
    this.loadRelatedData(() => {
      ...
      this.editForm.patchValue({
        usuarioCodigo: this.selectedItem.usuarioCodigo,
        usuarioNmbre: this.selectedItem.usuarioNmbre,
        usuarioContrasena: this.selectedItem.usuarioContrasena,
        usuarioEstado: this.selectedItem.usuarioEstado,
        codigoRol: this.selectedItem.codigoRol,
      });
  ...
  save() {
    ...
    params.usuarioCodigo = this.selectedItem.usuarioCodigo;

    this.spinner.show();
    this.apiService.usuarios.update(params).subscribe(() => {
        this.spinner.hide();
  ...
  onDeleteConfirmDialogYes() {
    ...
    this.apiService.usuarios.delete(this.selectedItem).subscribe(
      () => {
        this.spinner.hide();
  ...
  public loadRelatedData(finishedCallback) {
    ...
    this.apiService.pageData.usuarios().subscribe(
      (response: any) => {
        this.spinner.hide();
...
class UsuariosDataSource implements DataSource<any> {
