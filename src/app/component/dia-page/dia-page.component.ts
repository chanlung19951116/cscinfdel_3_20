...
@Component({
  selector: 'app-dia-page',
  templateUrl: './dia-page.component.html',
  styleUrls: ['./dia-page.component.scss']
})
export class DiaPageComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.dataSource = new DiaDataSource(this.apiService, this.spinner);
  ...
  initForms() {
    ...
    this.createForm = this.fb.group({
      ...
      descripcionDia: [null, Validators.compose([Validators.required])],
    });
  ...
  create() {
    ...
    this.apiService.dia.create(params).subscribe(() => {
        this.spinner.hide();
  ...
  startEditing(row) {
    ...
    this.editForm.patchValue({
      ...
      descripcionDia: row.descripcionDia,
    });
    this.editForm.markAsUntouched();

    this.editForm.controls['codigoDia'].disable();
  ...
  save() {
    ...
    this.editForm.controls['codigoDia'].enable();
    const params = this.editForm.value;
    params.codigo = this.selectedItem.codigo;


    this.spinner.show();
    this.apiService.dia.update(params).subscribe(() => {
        this.spinner.hide();
  ...
  onDeleteConfirmDialogYes() {
    ...
    this.apiService.dia.delete(this.selectedItem).subscribe(
      () => {
        this.spinner.hide();
...
class DiaDataSource implements DataSource<any> {
