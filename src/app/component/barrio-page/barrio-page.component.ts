...
@Component({
  selector: 'app-barrio-page',
  templateUrl: './barrio-page.component.html',
  styleUrls: ['./barrio-page.component.scss']
})
export class BarrioPageComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    this.dataSource = new BarrioDataSource(this.apiService, this.spinner);
  ...
  initForms() {
    ...
    const controlsConfig = {
      barrioDescripcion: [null, Validators.compose([Validators.required])],
      barrioEstado: [null, Validators.compose([Validators.required])],
  ...
  create() {
    ...
    this.spinner.show();
    this.apiService.barrio.create(params).subscribe(() => {
        this.spinner.hide();
  ...
  startEditing(row) {
    ...
    this.loadRelatedData(() => {
      ...
      this.editForm.patchValue({
        barrioDescripcion: this.selectedItem.barrioDescripcion,
        barrioEstado: this.selectedItem.barrioEstado,
  ...
  save() {
    ...
    params.barrioCodigo = this.selectedItem.barrioCodigo;

    this.spinner.show();
    this.apiService.barrio.update(params).subscribe(() => {
        this.spinner.hide();
  ...
  onDeleteConfirmDialogYes() {
    ...
    this.apiService.barrio.delete(this.selectedItem).subscribe(
      () => {
        this.spinner.hide();
  ...
  public loadRelatedData(finishedCallback) {
    ...
    this.apiService.pageData.barrio().subscribe(
      (response: any) => {
        this.spinner.hide();
...
class BarrioDataSource implements DataSource<any> {
